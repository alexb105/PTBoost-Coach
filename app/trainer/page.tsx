...
import { Shield, Plus, UserPlus, Mail, Phone, User, Copy, Check, Trash2 } from "lucide-react"
...
export default function TrainerPortalPage() {
  ...
  const fetchTrainerLimits = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setTrainerLimits({
          maxClients: data.trainer.maxClients,
          clientCount: data.trainer.clientCount,
        })
      }
    } catch (error) {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 403) {
          // Limit reached
          toast.error(data.error || "Client limit reached. Please upgrade your subscription.")
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setIsDeleting(true)
      const response = await fetch(`/api/admin/customers/${deleteCustomerId}`, {
        method: "DELETE",
        credentials: 'include',
      })
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            {trainerLimits !== null && 
             trainerLimits.maxClients !== 9999 && 
             trainerLimits.clientCount >= trainerLimits.maxClients && (
              <p className="text-sm text-destructive mt-1">
                Client limit reached ({trainerLimits.clientCount} / {trainerLimits.maxClients}). Upgrade to add more clients.
              </p>
            )}
          </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this customer? This action cannot be undone and will permanently delete all associated data including workouts, messages, and progress tracking.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
